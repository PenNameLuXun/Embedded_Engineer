...
> 在第 07 章我们用一份"刚刚能跑"的 `startup.s` 和 `linker.ld`。这一章把它们升级到生产级：复位时初始化 `.data` / 清 `.bss`、放 240 个中断槽位、把 `printf` 接到 UART —— 顺便彻底搞懂 GNU `ld` 链接脚本的语法。
>
> **学完本章你应该能**：(1) 看懂任何一份 GCC 风格的 Cortex-M 链接脚本，(2) 从零写一份完整启动文件，(3) 解释复位向量被 CPU 怎么用、`.data` 怎么从 Flash 搬到 SRAM。
...
```
1. 从 0x0000_0000 读 32 位 → 放进 MSP（栈顶）
2. 从 0x0000_0004 读 32 位 → 放进 PC（复位向量）
...
_vectors:
    ...
    /* 之后是 240 个外设 IRQ，全部默认指向 Default_Handler */
    .rept 240
...
**`.weak` 的精妙之处**：这些 handler 都是弱符号，用户随时可以在 C 里写 `void SysTick_Handler(void) { ... }` 来覆盖它。没覆盖时连接到 `Default_Handler`（死循环）。
...
写到 ELF header 里。GDB 启动时用这个符号定位入口；多数 loader 用向量表自动找，但 ELF 里有 entry 是好习惯。
...
- **VMA vs LMA**：VMA = Virtual Memory Address，运行时 CPU 访问的地址；LMA = Load Memory Address，烧到 Flash 时存哪。`.data` 是经典的 VMA ≠ LMA。
...
`*.map` 文件（`-Wl,-Map=`）告诉你每个符号 / 段最终放在哪个地址、占多少字节。**读 .map 文件是调代码尺寸 / 调链接错误的基本功**。
...
| 概念              | 下游章节                                    |
|-------------------|---------------------------------------------|
| 向量表 weak 符号  | [11 中断](../11_中断与异常/) 用 override 写 ISR |
| `.data` / `.bss`  | [13 DMA](../13_DMA/) 涉及内存属性             |
| MSP / PSP         | [25 FreeRTOS 实战](../25_FreeRTOS实战/)        |
| MPU + 链接段属性  | [40 嵌入式安全](../40_嵌入式安全/)             |

下一章 [10 第一个程序：寄存器级 GPIO/UART](../10_第一个程序_GPIO/) 在这些底座上写一个真正初始化时钟、GPIO、UART 的程序。
