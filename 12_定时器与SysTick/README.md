...
> 时间是嵌入式工程师手里的硬通货。本章建立"精确时基"：用 SysTick 做毫秒节拍、用通用定时器做更高分辨率事件、用 PWM 输出生成波形。
>
> **学完本章你应该能**：(1) 配置 SysTick 实现 1 ms 节拍并写 `delay_ms`，(2) 解释定时器三大用法（计数、比较输出、捕获输入），(3) 知道为什么用定时器 + DMA 能做精确波形发生器（铺垫 13 章）。
...
| 来源              | 分辨率           | 用途                                |
|-------------------|------------------|-------------------------------------|
| **SysTick**       | 1 CPU 周期       | OS 节拍、`delay_ms`                  |
...
void timer0a_periodic_init(uint32_t bus_hz, uint32_t hz)
{
    ...
    *(volatile uint32_t *)(TIMER0_BASE + GPTM_IMR)  = 0x1;      /* TimeOut IRQ */
    *(volatile uint32_t *)(TIMER0_BASE + GPTM_CTL)  = 0x1;      /* TAEN */

    /* NVIC 使能 Timer0A IRQ = 19 */
...
具体代码见 `code/05_pwm/main.c`（QEMU 不会驱动真 GPIO 波形，但寄存器值可以从 GDB 看出来）。
