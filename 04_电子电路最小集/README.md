...
**基尔霍夫两定律**（背一下足够）：
- KCL（Kirchhoff's Current Law，基尔霍夫电流定律）：进一个节点的电流总和 = 出的总和。
- KVL（Kirchhoff's Voltage Law，基尔霍夫电压定律）：闭环里电压代数和 = 0。
...
**用途**：
- 把 5 V 信号降到 MCU（Microcontroller Unit，微控制器单元）能吃的 3.3 V（短期、低速场合）
- 给 ADC（Analog-to-Digital Converter，模数转换器）喂一个固定参考
- 配合温敏电阻 / 光敏电阻当传感器
...
数字 IO 引脚的四种典型驱动方式，理解这四种 = 解锁 80% 的 GPIO（General Purpose Input/Output，通用输入/输出）/ 总线问题。

### 推挽（Push-Pull）

两个 MOS 一上一下，一个把脚拉到 VCC，另一个把脚拉到 GND。CMOS（Complementary Metal-Oxide-Semiconductor，互补金属氧化物半导体）电路的标准输出结构就是推挽，两个互补的晶体管交替导通，像两个人一起推拉一根绳子。
...
- **用途**：单端、点对点的输出，比如点 LED、驱动 SPI（Serial Peripheral Interface，串行外设接口）MOSI。

### 开漏（Open-Drain）/ 开集（Open-Collector）

只有下面那颗 NMOS，上面没了。引脚只能主动拉低，**释放时是高阻（Hi-Z）**。"开漏"的意思是 MOSFET 的漏极（Drain）悬空开路，没有内部上拉。
...
- **用途**：I²C（Inter-Integrated Circuit，集成电路互联总线）、CAN（Controller Area Network，控制器局域网）、中断聚合（open-drain interrupt）、多主总线。
...
| 一线开漏中断       | 中断源    | 开漏      | 必须                  |
| CAN_H / CAN_L      | CAN PHY（Physical Layer Transceiver，物理层收发器）   | 差分推挽  | （差分有终端电阻）      |
| SPI 各信号         | 主或从    | 推挽      | 不需要                |
...
为什么需要？数字 IC 一开关，瞬间电流可达几百 mA，持续几 ns。电源走线有寄生电感 → `V = L × di/dt` → 电压下垂。去耦电容就在身边随时供能，相当于在 IC 旁边放了一个"小型储能装置"，能在供电电流突变的瞬间提供补充。
...
**电解电容有极性**，反接会爆（字面意义）。**陶瓷电容无极性**。看 PCB（Printed Circuit Board，印刷电路板）上的标记。
...
代表：GPIO、UART（Universal Asynchronous Receiver/Transmitter，通用异步收发传输器）、SPI、I²C。
...
代表：USB、Ethernet、HDMI、PCIe、MIPI、CAN、LVDS（Low-Voltage Differential Signaling，低电压差分信号）、差分时钟。
...
- SWD / JTAG（Joint Test Action Group，联合测试行动组，一种调试接口标准）引出排针：调试口
