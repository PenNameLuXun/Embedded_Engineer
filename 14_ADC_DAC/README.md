...
> 嵌入式系统终究要和真实世界交互 —— 温度、声音、电流、电压。ADC（模数转换器）把它们数字化；DAC（数模转换器）把数字打回模拟。这一章建立**采样率、量化噪声、SAR/Σ-Δ 两种 ADC 拓扑**的概念，给一段最小代码。
>
...
参数三件套：
- **分辨率 (bits)**：8/10/12/16/24，越高分得越细
- **采样率 (sps, samples per second)**：1k / 1M / 1G
- **参考电压 (Vref)**：ADC 输入 0~Vref 映射到 0~2ⁿ-1

LSB 大小 = Vref / 2ⁿ。例：12 位 ADC、Vref=3.3 V → 1 LSB = 0.8 mV。
...
如果违反，就发生 **混叠 (aliasing)**：高频信号被"折射"成低频，**信息丢失且不可逆**。
...
把连续值"舍入"到 2ⁿ 个离散值，引入 ±0.5 LSB 的量化误差。统计上等效一个白噪声，功率 = (LSB)² / 12。

理想 n 位 ADC 的 **SNR (Signal-to-Noise Ratio)**：
...
实际 ADC 的有效位数 **ENOB (Effective Number Of Bits)** 通常比标称低 1–3 位，因为真实电路有热噪声、非线性、时钟抖动 (jitter)。

**ENOB 才是 ADC 的"真分辨率"**，标称只是市场宣传。
...
10 位 SAR，4 通道复用，最大 500 ksps。核心概念是 **Sample Sequencer (SS)**：
...
DAC 比 ADC 简单。常见拓扑：
- **R-2R 网络**：用电阻分压网络把数字转电压。简单、单调
- **电流舵 (Current-Steering)**：高速 DAC 主流，DDS / 通信用
- **PWM + RC 滤波**：MCU 没有专用 DAC 时的廉价替代品，但精度低
...
- Ping-pong：一个 buffer 装满时 DMA 切到另一个，CPU 处理刚装满那个
...
| [11 中断与异常](../11_中断与异常/) | ISR、NVIC、临界区     |
