...
> **USB（Universal Serial Bus，通用串行总线）** 是嵌入式领域里**最复杂**的协议之一 —— 一个完整 USB 主机栈代码量轻松超过 10 万行。这一章不教你写驱动，但让你理解：**插上一根 USB 线时背后发生了什么**，descriptor 和 enumeration 是什么，class driver 是什么意思。
>
> **学完本章你应该能**：(1) 解释 USB 的拓扑（Host-centric）和事务模型，(2) 描述设备插入时的枚举过程，(3) 知道四种 transfer 类型和它们的取舍，(4) 看到 CDC/HID/MSC/UVC 知道是什么 class。

---

![USB概览配图](images/enumeration.png)

## 19.1 USB 的世界观：Host-centric

USB 不是 peer-to-peer（点对点）。**总是有一个 Host（主机）**，连接 N 个 **Device（设备）**。设备只有被询问时才发数据 —— 这是 USB 简化协议、降低设备硬件成本的核心选择。类比：就像老师点名提问，学生等被叫到才回答，而不是随时抢着说话。
...
![19.1 USB 的世界观：Host-centric](images/generated/usb_host_tree.png)

- 1 个 Host 最多 127 个设备（地址 7 位）
- 树形拓扑，HUB 中转
- USB-C 引入了 USB-PD（电源协商）和"双角色"（设备可以变 Host，即 **OTG（On-The-Go，USB移动互联标准，允许设备切换主从角色）**）但模型基本未变
...
**USB 2.0 / FS / LS 共用 D+ / D-** 差分对。**USB 3.0+ 额外加 SuperSpeed 对** 物理上分离，向后兼容靠 mux（多路选择器）。

物理层细节属于"**PHY（Physical Layer Transceiver，物理层收发器）** 工程"，本章不展开。
...
![19.3 设备枚举：插上之后](images/generated/usb_enumeration_flow.png)

第 3–6 步全部通过 **端点 0 控制传输** 走。这是为什么所有 USB 设备都得实现 **EP0 = Default Control Pipe（默认控制管道）**。枚举完成后，你的电脑上才会出现"新设备已就绪"的提示。
...
![19.4 Descriptor：设备的"自我介绍"](images/generated/usb_descriptor_hierarchy.png)

最重要几个字段：

**Device Descriptor**:
- `idVendor` / `idProduct`：VID（Vendor ID，厂商ID） / PID（Product ID，产品ID），全球唯一识别设备厂商和型号（VID 要花钱买）
...
| **Bulk**       | U 盘、打印机、串口转换器     | 大块数据，可被挤兑（没有带宽保证）  |
| **Isochronous**| 音频、视频                  | 固定带宽，**不保证数据完整性**，宁可丢包也不补传 |

设备申报"我要 X kbps 带宽"，Host 按总带宽预算分配。**总带宽预算才是 Host 排程的核心约束**。Isochronous 用于音频是因为音频需要稳定的时间间隔，偶尔丢一帧比延迟更可接受。
...
| Class        | 缩写  | 例子                  |
|--------------|-------|-----------------------|
| **HID（Human Interface Device，人机接口设备）** | 0x03  | 键盘、鼠标、游戏手柄   |
| Mass Storage | 0x08  | U 盘、SD 读卡器（即 **MSC，Mass Storage Class，大容量存储类**）|
| **CDC（Communications Device Class，通信设备类）** | 0x02  | 虚拟串口、网卡         |
...
MCU（Microcontroller Unit，微控制器单元）当设备端，靠 USB Device IP + 外部 PHY 接 D+/D-：
...
Cortex-A 类 SoC（System on Chip，片上系统）自带 EHCI/xHCI 主控制器。MCU 上偶尔有 OTG 控制器能切到 host 模式。Host 复杂度比 device 高一个数量级（要管理 hub 树、多设备调度、电源协商）。

### ③ USB OTG

设备能在 host 和 device 间切换。手机典型用法：接电脑时当 device（U 盘 / 充电），接 U 盘时切换成 host。
...
![19.8 协议栈分层](images/generated/usb_stack_layers.png)
