...
> 上一章 echo 程序里，main 一直在死循环里轮询 UART（Universal Asynchronous Receiver/Transmitter，通用异步收发传输器）—— **CPU（Central Processing Unit，中央处理器）100% 占用却 99% 时间在等**。这是嵌入式工程师入门的第一个真正要解决的问题。这一章把"轮询"换成"中断"，CPU 才能去干别的事。
>
> **学完本章你应该能**：(1) 解释中断和异常的差别、Cortex-M 异常进出流程，(2) 写一个 UART RX 中断处理，(3) 处理 ISR（Interrupt Service Routine，中断服务例程）与主程序共享状态的同步问题，(4) 知道什么是中断延迟、为什么 ISR 要尽可能短。
...
- **异常 (Exception)**：CPU 因为内部原因被打断（除零、非法指令、SVC（SuperVisor Call，特权模式调用指令）调用）。
- **中断 (Interrupt / IRQ（Interrupt ReQuest，中断请求）)**：CPU 因为外部事件被打断（UART 收到字节、定时器到期、引脚电平变化）。
- **故障 (Fault)**：异常的子集，特指出错型（HardFault（硬件错误异常）、BusFault、UsageFault、MemManage）。

在 Cortex-M 里，三者**走的是同一套机制 + 同一张向量表**，只是优先级不同。这就是为什么第 09 章那张向量表里 16 + 240 个槽位混在一起。

> **类比理解**：把 CPU 想象成一个正在处理文件的员工。异常是他自己出了问题（发现文件格式错误，得停下来处理）；中断是门铃响了（外部事件，UART 有新数据），得去开门；故障是撞到了墙（严重错误）。Cortex-M 的聪明之处在于，无论哪种情况，处理流程是统一的一套机制。
...
1. 当前正在执行的指令做完。
2. 把 8 个寄存器自动压栈：xPSR、PC（Program Counter，程序计数器）、LR（Link Register，链接寄存器）、R12、R3、R2、R1、R0 —— 这叫 **stacking**。
...
## 11.3 NVIC（Nested Vectored Interrupt Controller，嵌套向量中断控制器）：让 IRQ 真的"通"过来
...
少一条它就憋着不动。**新手 90% 的"中断没进"问题**都是这三个之一。把它记成"三道闸门"：任何一道没打开，中断信号就过不来。

### NVIC API（CMSIS（Cortex Microcontroller Software Interface Standard，Cortex微控制器软件接口标准））
...
负数 IRQn 是系统异常（SysTick（System Tick Timer，系统滴答定时器）、SVC、PendSV（Pendable SerVice，可挂起服务调用）），正数才是外设。**`SysTick_IRQn = -1`**，所以你不会在 `NVIC_EnableIRQ` 里看到它，SysTick 由 `SYST_CSR.TICKINT` 自己使能。
...
> **为什么第一步必须清中断标志？** 大多数外设的中断是"电平触发"而不是"脉冲触发"：只要中断标志寄存器的位还是 1，外设就一直向 NVIC 报中断。如果 ISR 退出时不清这个标志，CPU 刚回到主程序就又被打断，无限循环进 ISR，永远无法执行主程序代码。
...
int main(void)
{
    ...
    int led = 0, n = 0;
    while (1) {
        /* 没事就 WFI（Wait For Interrupt，等待中断，低功耗指令）睡觉，节能 */
        if (rx_tail == rx_head) {
            __asm__("wfi");
            continue;
        }
...
上面那段 ring buffer：head 由 ISR 写，tail 由 main 写，**双方都读对方的指针**。这是经典的 **SPSC (single-producer single-consumer)** 队列，即"单生产者单消费者"。
...
`__disable_irq` 关所有 IRQ（除 NMI（Non-Maskable Interrupt，不可屏蔽中断）/HardFault（硬件错误异常））。**`BASEPRI`** 只屏蔽优先级**数字大于等于** N 的，比 N 小（更高优先级）的还能进：
...
**中断延迟 (Interrupt Latency)** = 中断信号产生 → ISR（中断服务例程）第一条指令执行之间的周期数。
...
5. **优先级要规划**：高速 / 实时性强的中断 (DMA（Direct Memory Access，直接内存访问）done、Timer overflow) 给高优先级；UART RX 中等；GPIO（General Purpose Input/Output，通用输入/输出）边沿一般低。
...
下一章 [12 定时器与 SysTick](../12_定时器与SysTick/) 在 ISR 基础上构建精准时基和 PWM（Pulse Width Modulation，脉冲宽度调制）输出。
