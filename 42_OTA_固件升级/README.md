...
> 现场几万台设备总会有漏洞、有新功能要推。**OTA（Over-The-Air，空中升级，通过无线网络远程更新固件）** = 远程升级。这一章看 A/B 分区、差分升级、回滚的标准做法。
>
...
**FOTA（Firmware Over-The-Air，固件空中升级）** 是 OTA 在固件领域的专用术语。与手机 App 升级不同，嵌入式固件升级面临更严苛的约束：设备远在外面，升级时不能假设：
- 网络稳定（中途断电、丢包）
...
**防砖设计**是 OTA 的核心工程挑战：砖（brick）指设备因升级失败导致无法正常启动、无法再次升级的状态。防砖设计的基本原则是：**升级过程中始终保留一份可用的固件**，任何失败场景都能自动回退。**回滚（rollback）**：升级失败时自动恢复到上一个可用版本，是防砖的最后一道防线。

---

## 42.2 A/B 分区方案

**A/B 分区升级（双分区方案：保留当前运行分区，将新固件写入另一分区，升级成功后切换）** 是目前最主流的防砖设计：
...
**MCUBoot（一个用于 MCU 的安全引导加载程序框架，Zephyr 和 NuttX 的默认 bootloader）** 专为资源受限的 MCU 设计，内置签名验证、A/B 分区切换和回滚机制。MCU 主流：**MCUboot**；Linux 主流：**SWUpdate / RAUC / OSTree**。
...
**SHA256**（SHA-2 家族的 256 位版本，输出 32 字节摘要，用于验证固件完整性）是最常用的固件哈希算法。**CRC（Cyclic Redundancy Check，循环冗余校验）** 也用于完整性检查，但只能防误码不能防篡改，不能替代签名验证。
